    ...
    public partial class LoginBox : System.Web.UI.UserControl
    {
        ...
        void BindCodes()
        {
            try
            {
                ...
                list = System.Threading.Thread.CurrentThread.CurrentCulture.Name.ToLower() == "ar-kw" ? list.OrderBy(c => c.TitleAR).ToList() : list.OrderBy(c => c.TitleEN).ToList();
            ...
            try
            {
                ...
                if (userObj != null)
                {
                    ...
                    if (userObj.Password == AuthHelper.GetMD5Hash(txtPassword.Text))
                    {
                        if (userObj.IsActive)
                        {
                            BasePage pageObj = this.Page as BasePage;
                            if (pageObj != null)
                                pageObj.UserId = userObj.Id;
                            ...
                            PresentHelper.ShowScriptMessage(Resources.Resource.welcome + " " + userObj.FullName + " " + Resources.Resource.open_your_account);
                        }
                        else
                        {
