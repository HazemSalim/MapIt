...
{
    public class App_ServiceComment
    {
        ...
        public App_ServiceComment(ServiceComment comment, long serviceId, long receiverId)
        {
            var serviceCommentsRepository = new ServiceCommentsRepository();

            Id = comment.Id;

            ServiceId = comment.ServiceId;
            ServiceUserId = comment.Service.UserId;
            ...
            UnreadMessageCount = serviceCommentsRepository.Count(c => !c.IsRead && c.ServiceId == serviceId && c.ReceiverId == receiverId &&
                (receiverId == comment.SenderId ? c.SenderId == comment.ReceiverId : c.SenderId == comment.SenderId));
            AddedOn = comment.AddedOn;
        }

        public App_ServiceComment(ServiceComment comment, long receiverId)
        {
            var serviceCommentsRepository = new ServiceCommentsRepository();
            ...
            TextMessage = comment.TextMessage;
            IsRead = comment.IsRead;
            UnreadMessageCount = serviceCommentsRepository.Count(c => !c.IsRead && c.ReceiverId == receiverId &&
                (receiverId == comment.SenderId ? c.SenderId == comment.ReceiverId : c.SenderId == comment.SenderId));
            AddedOn = comment.AddedOn;
        }
    }
